...
public class Cannon {
    ...
    public void draw(Canvas canvas) {
        ...
        if ((cannonball != null) && (cannonball.velocityX > 0)) {
            paint.setStrokeWidth(5); // set width of path
            paint.setStyle(Paint.Style.STROKE);
            paint.setPathEffect((new DashPathEffect(new float[]{10, 5}, 0)));
            paint.setColor(Color.BLUE);
            canvas.drawLine((float) barrelEnd.x, (float) barrelEnd.y, cannonball.shape.exactCenterX(),
                    cannonball.shape.exactCenterY(), paint);
            paint.setStrokeWidth(this.barrelWidth);
